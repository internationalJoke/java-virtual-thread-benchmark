...
public class ParallelQuickSort {
    ...
    public static void sort(int[] array, ThreadMode mode) {
        sort(array, mode, DEFAULT_MAX_DEPTH, DEFAULT_SEQUENTIAL_THRESHOLD);
    }
    ...
    public static void sort(int[] array, ThreadMode mode, int maxDepth, int sequentialThreshold) {
        ...
        if (mode == ThreadMode.PLATFORM) {
            ...
            virtualQuickSort(array, 0, array.length - 1, effectiveThreshold, 0, maxDepth);
        }
    }
    ...
    private static void virtualQuickSort(int[] array, int low, int high, int sequentialThreshold, int depth, int maxDepth) {
        if (low >= high) return;
        ...
        Thread leftThread = Thread.ofVirtual().start(() ->
                virtualQuickSort(array, low, bounds[0] - 1, sequentialThreshold, depth + 1, maxDepth));

        virtualQuickSort(array, bounds[1] + 1, high, sequentialThreshold, depth + 1, maxDepth);
